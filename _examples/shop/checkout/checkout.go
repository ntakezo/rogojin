// Package checkout is the checkout workflow of the shop domain. It
...
// another domain may hold a checkout of its own.
const Name = "shop/checkout"
...
func (w workflow) ValidateInput(input any) error {
	if _, ok := input.(states.Input); !ok {
		return fmt.Errorf("shop/checkout: expected states.Input, got %T", input)
	}
	return nil
}
...
func (w workflow) NewInstance(input any, deps workflows.Deps) (workflows.Instance, error) {
	...
	if !ok {
		return nil, fmt.Errorf("shop/checkout: expected states.Input, got %T", input)
	}
