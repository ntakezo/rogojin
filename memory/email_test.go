...
// TestEmailsSaveListRoundTrip verifies an email round-trips whole — address,
// inbox credentials, cursor — in stable id order, and an address-only email
// reads back with no inbox.
...
// TestEmailsVendorlessInboxStoresAsNone verifies an inbox without a vendor
// stores as no inbox at all — the address-only marker the columns produce.
func TestEmailsVendorlessInboxStoresAsNone(t *testing.T) {
	repo := NewEmails()
	ctx := context.Background()
...
// TestEmailsCursorUpsertPreservesCreatedAt verifies a cursor advance rewrites
// the record without revising when it was created.
...
// TestEmailsDeleteIsIdempotent verifies deletes remove the record and absent
// ids are a no-op.
...
// TestEmailsInboxDoesNotAliasTheStore verifies the caller's inbox pointer
// and the store's never share memory.
func TestEmailsInboxDoesNotAliasTheStore(t *testing.T) {
	repo := NewEmails()
	ctx := context.Background()
