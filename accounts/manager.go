...
//
//	tasks.WithResource("account", m.Unlock, accounts.StaleLockReleaser(m))
//
// An empty groupID means no group rather than the global one, exactly as it
// does for ReleaseStaleLock.
func StaleLockReleaser(m *Manager) func(ctx context.Context, taskID, groupID, accountID string) error {
	return func(ctx context.Context, taskID, groupID, accountID string) error {
		return m.ReleaseStaleLock(ctx, Assignment{TaskID: taskID, GroupID: groupID, AccountID: accountID})
	}
}
