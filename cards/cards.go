...
// package specialized to one model: a Card embeds leasing.Resource — the
// pool, group, and lock fields every leasable kind shares — and
// adds the card's fields: number, expiry, CVV, billing address, whatever the
...
// cards register no strategy of their own: a card is one instrument with one
...
// Treat a leased card's Fields as read-only — the pool shares the backing
// bytes.
package cards
...
// A Card is the durable record of one payment instrument: the leasing core
...
type Repository = leasing.Repository[Card]
