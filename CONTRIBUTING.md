...
A JSON request body types the `XRequest` struct and is marshaled back out of it, the way `_examples`' `submit_checkout.go` does by hand. `bodyTypers` in `internal/scaffold/body.go` dispatches by content type exactly as the response typers do; anything nothing handles keeps its captured bytes as a literal, which is always exact.

A request body differs from a response in one way that drives the whole file: **it goes back on the wire**, so two things a response never has to care about become correctness issues.

`encoding/json` writes struct fields in declaration order, and go-jsonstruct sorts them alphabetically — so `captureKeyOrder` walks the raw body's tokens to record the order each object's keys appeared in, and `orderAsCaptured` sorts the generated fields back into it at every level. Without that, a captured `{"challengeSessions":…,"layoutName":…}` goes out reordered. For the same reason a request body never collapses to a `map` however wide it is: `encoding/json` sorts a map's keys.
...
The exported `XResponse` type in a captured request is inferred from the captured response body, so a state can unmarshal into it without anyone reading the payload by hand. What "typed" means depends on the media type — a JSON object is a struct, an HTML document has no shape at all — so `internal/scaffold/response.go` dispatches through `typers`, a list of `{matches, render}` pairs. **Supporting another media type means appending one entry**; nothing else in the renderer knows what a response looks like.

The JSON typer wraps [go-jsonstruct](https://github.com/twpayne/go-jsonstruct), which does the inference: array elements are *merged* rather than sampled, so an endpoint that omits a key on some elements still contributes it, and — the reason to use a library here rather than hand-roll one — a key that was absent from some observations earns an `omitempty` and, for objects, a pointer. A generated response says which fields the endpoint actually guarantees. RFC 3339 strings become `time.Time`, which is why an inferred type reports the imports it needs and the template has a conditional import block.

That generator emits a whole file, so `inlineType` reduces it to the bare type expression the request file declares beside its function. `normalizeType` then fixes three things it will not do for us, each of which came from a real capture:

- **Empty JSON keys.** `DefaultExportNameFunc` *panics* on a key with no components, and `WithSkipUnparsableProperties` does not prevent it. `exportFieldName` never returns the empty string, and the resulting `json:""` field — which Go would silently bind under the field's own name — is dropped from the AST. `jsonType` also recovers, so a body that trips an edge case we have not found degrades to an untyped response instead of killing the command.
- **Colliding names.** The generator does not dedupe: two keys reducing to one identifier emit the same field twice and do not compile.
- **Dictionaries.** An object of at least `dictionaryFields` same-shaped keys becomes a `map[string]T`. A capture used in testing returned a translation payload of 222 keys; a struct that wide misses every key the capture happened not to contain and cannot be looked up by a value known only at run time.

Field naming is ours, via `WithExportNameFunc` — `fieldIdent` runs the components together and capitalizes initialisms whole, so a key gives `CartID` and `ImageURL` rather than the generator's default `Cart_Id`. Fields sort by JSON key, not by the Go name derived from it.
